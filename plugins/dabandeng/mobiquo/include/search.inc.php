...
//    }
    ...
    if(!empty($searchid)) {
        ...
        $query = $sdb->query("SELECT t.*,p.message
                              ...
                              WHERE t.tid=p.tid AND t.author=p.author AND t.dateline=p.dateline AND
                                    t.tid IN ($index[tids]) AND t.displayorder>='0' 
                              ORDER BY t.$orderby $ascdesc LIMIT $start_limit, $tpp");
        while($thread = $sdb->fetch_array($query)) {
            $thread['dbdateline'] = $thread['dateline'];
            $thread['dblastpost'] = $thread['lastpost'];
            $threadlist[] = procthread($thread);
