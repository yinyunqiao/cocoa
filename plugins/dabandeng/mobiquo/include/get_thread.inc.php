...
$ppp = $end_num - $start_num + 1;
$start_limit = $start_num;
$page = $start_num/$ppp + 1;
...
$postlist = $attachtags = $attachlist = $threadstamp = $attachedids = array();
$aimgcount = 0;
$attachpids = -1;

$thisgid = $forum['type'] == 'forum' ? $forum['fup'] : $_DCACHE['forums'][$forum['fup']]['fup'];
$lastmod = $thread['moderated'] ? viewthread_lastmod() : array();
if($version != '7.1' && !$threadstamp && @getstatus($thread['status'], 5)) {
...
while($post = $sdb->fetch_array($query)) {
    if(($onlyauthoradd && $post['anonymous'] == 0) || !$onlyauthoradd) {
        ...
        // added 
        $postlist[$post['pid']]['needhiddenreply'] = ($hiddenreplies && $discuz_uid != $post['authorid'] && $discuz_uid != $thread['authorid'] && !$post['first'] && !$forum['ismoderator']);
    }
}

if($savepostposition && $positionlist && $version != '7.1') {
...
if(isset($hookscript['viewthread']['module']['dps_preply']))
{
    $hookscript['global'] = array();
    $hookscript['viewthread']['module'] = array('dps_preply' => $hookscript['viewthread']['module']['dps_preply']);
    $hookscript['viewthread']['outputfuncs'] = array('viewthread_postbottom' => $hookscript['viewthread']['outputfuncs']['viewthread_postbottom']);
    foreach($hookscript['viewthread']['outputfuncs']['viewthread_postbottom'] as $keyid => $dps_preply_data)
    {
        if ($dps_preply_data[0] != 'dps_preply')
        {
            unset($hookscript['viewthread']['outputfuncs']['viewthread_postbottom'][$keyid]);
        }
    }
    @hookscriptoutput('');
}
...
function viewthread_procpost($post, $special = 0) {
    ...
    global $_DCACHE, $newpostanchor, $numpost, $thisbg, $postcount, $ratelogpids, $onlineauthors, $lastvisit, $thread,
        $attachpids, $attachtags, $attachedids, $forum, $dateformat, $timeformat, $timeoffset, $userstatusby, $allowgetattach,
    ...
    if($post['username']) {
        ...
        if($showavatars) {
            $GLOBALS['avatarmethod'] = '';
            $post['avatar'] = discuz_uc_avatar($post['authorid'], '', true);
    ...
    if($post['attachment']) {
        if($allowgetattach && !$threadpay) {
            ...
            if(preg_match_all("/\[attach\](\d+)\[\/attach\]/i", $post['message'], $matchaids)) {
                ...
                $attachedids[$post['pid']] = $matchaids[1];
            }
        } else {
    ...
    $post['smileyoff'] = 1;
    $post['message'] = discuzcode($post['message'], $post['smileyoff'], $post['bbcodeoff'], $post['htmlon'] & 1, $forum['allowsmilies'], $forum['allowbbcode'], ($forum['allowimgcode'] && $showimages ? 1 : 0), $forum['allowhtml'], 0, 0, $post['authorid'], $forum['allowmediacode'], $post['pid']);
...
function viewthread_lastmod() {
    global $db, $tablepre, $dateformat, $timeformat, $threadstamp, $timeoffset, $tid, $_DCACHE, $version;
    $version == '7.1' ? $stamp_add = '' : $stamp_add = ', stamp';
    if($lastmod = $db->fetch_first("SELECT uid AS moduid, username AS modusername, dateline AS moddateline, action AS modaction, magicid".$stamp_add."
        ...
        $lastmod['modaction'] = ($modactioncode[$lastmod['modaction']] ? $modactioncode[$lastmod['modaction']] : '').($version == '7.1' ? '' :($lastmod['modaction'] != 'SPA' ? '' : ' '.$_DCACHE['stamps'][$lastmod['stamp']]['text']));
        if($lastmod['magicid']) {
            require_once DISCUZ_ROOT.'./forumdata/cache/cache_magics.php';
