...
//    }
    ...
    $sqlsrch = "FROM {$tablepre}posts p, {$tablepre}threads t WHERE $digestltd t.fid IN ($fids) $topltd AND p.tid=t.tid AND p.invisible='0'";
    ...
    $threads = $tids = 0;
    $query = $sdb->query("SELECT DISTINCT t.tid, t.closed, t.author $sqlsrch ORDER BY t.lastpost DESC LIMIT $start_num, $limit_num");
...
$threadlist = array();
$query = $sdb->query("SELECT t.*,p.message,p.pid,p.authorid,p.author
                      FROM {$tablepre}threads t, {$tablepre}posts p 
                      WHERE t.tid=p.tid AND t.lastposter=p.author AND t.lastpost=p.dateline AND 
                            t.tid IN ($index[tids]) AND t.displayorder>='0' 
